...
public class FrenzyTransform : MonoBehaviour {
    ...
    public void OnChangeFrenziedState (InputAction.CallbackContext context) {
        ...
        if (!gameObject.GetComponent<PlayerPause>()._pauseMenu.activeSelf && gameObject.GetComponent<PlayerController>()._activeTransformCooldown <= 0 && gameObject.GetComponent<PlayerController>()._isFrenzied && gameObject.GetComponent<PlayerController>().GetFrenzyMeter() >= (float)(gameObject.GetComponent<PlayerController>().GetFrenzyMeterMax() - 27)) { 
